  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      buildingData.forEach((b, i) => {
        const mat = new THREE.MeshStandardMaterial({ color:buildingColors[i%buildingColors.length], roughness:0.3, metalness:0.6, emissive:buildingColors[i%buildingColors.length], emissiveIntensity:0.15 });
        ...
        for (let row=1; row<Math.floor(b.h/2); row++) {
          for (let col=-1; col<=1; col++) {
            if (Math.random()>0.4) {
              const wMat = new THREE.MeshStandardMaterial({ color:Math.random()>0.3?0xffd54f:0x64b5f6, emissive:Math.random()>0.3?0xffd54f:0x64b5f6, emissiveIntensity:1.5 });
      ...
      function makeCar(color: number) {
        ...
        const cab=new THREE.Mesh(new THREE.BoxGeometry(0.8,0.4,0.65),new THREE.MeshStandardMaterial({color,roughness:0.2})); cab.position.set(-0.1,0.4,0); g.add(cab);
        return g;
      }
      function makeMetro() {
        const g = new THREE.Group();
        [0,2.4,-2.4].forEach(xOff => { const c=new THREE.Mesh(new THREE.BoxGeometry(2.0,0.7,0.7),new THREE.MeshStandardMaterial({color:0x3b82f6,roughness:0.2,metalness:0.8,emissive:0x1d4ed8,emissiveIntensity:0.2})); c.position.x=xOff; g.add(c); });
        return g;
      }
      ...
      function animate() {
        ...
        rings.forEach(ring=>{ ring.userData.angle+=ring.userData.speed; ring.position.x=Math.cos(ring.userData.angle)*ring.userData.radius; ring.position.z=Math.sin(ring.userData.angle)*ring.userData.radius-5; ring.position.y=ring.userData.baseY+Math.sin(t*0.8+ring.userData.yOff)*0.5; ring.rotation.x=t*0.3; ring.rotation.y=t*0.2; });
        camera.position.x=Math.sin(t*0.1)*2; camera.position.y=14+Math.sin(t*0.07)*1; camera.lookAt(0,2,0);
        cityGlow.intensity=2.5+Math.sin(t*1.5)*0.5; stars.rotation.y=t*0.005;
    ...
    init();
    return ()=>{ cancelAnimationFrame(animId); if(renderer){ renderer.dispose(); try{ if(mountRef.current&&renderer.domElement.parentNode===mountRef.current) mountRef.current.removeChild(renderer.domElement); }catch{} } };
  }, []);
